/**
 * Этот код определяет Redux slice для управления состоянием пользователя в приложении.
 * Он содержит логику для хранения данных о текущем пользователе, статусе аутентификации и
 * работе с токеном авторизации. Slice также предоставляет редюсеры и обработчики для различных
 * сценариев, таких как успешный логин, получение текущего пользователя или получение пользователя по ID.
 * Это позволяет централизовать управление состоянием пользователя и облегчает работу с асинхронными
 * операциями, связанными с пользователями, такими как логин и регистрация.
 */
